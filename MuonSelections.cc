...
bool muonID(unsigned int muIdx, id_level_t id_level){
  ...
  switch (id_level){
   ...
   case(SS_tight_v4):
      if (muonID(muIdx, SS_tight_noiso_v3)==0) return false;
      if (muIDCacheSet) return passMultiIso(0.14, 0.73, 6.7, muID_cache.getMiniiso(muIdx), muID_cache.getPtratio(muIdx), muID_cache.getPtrel(muIdx) );
      else return passMultiIso(13, muIdx, 0.14, 0.73, 6.7);
      break;
...
int muTightID(unsigned int muIdx, analysis_t analysis, int version){
  switch (analysis){
    case (POG):
      ...
      break;
    case (WW):
      if (muonID(muIdx, WW_medium_v1)) return 2;
      if (muonID(muIdx, WW_fo_v1))     return 1;
      if (muonID(muIdx, WW_veto_v1))   return 0;
      break;
    case (SS):

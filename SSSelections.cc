...
vector <pair<GenParticleStruct, GenParticleStruct> > makeGenHyps(){
  ...
  for (int iGen = 0; (unsigned)iGen < tas::genps_p4().size(); iGen++) {
    ...
    if (abs(id) == 11 || abs(id) == 13) {
      ...
      nLep++; 
      //cout << tas::genps_status().at(iGen) << " " << tas::genps_id_mother().at(iGen) << " " << tas::genps_p4().at(iGen).pt() << " " << tas::genps_p4().at(iGen).eta() << endl;
    }
  ...
  if (nLep == 0){
    cout << "no leptons found" << endl;
    for (unsigned int i = 0; i < tas::genps_id().size(); i++){
      cout << Form("%5i %5i % 10.2f %5i %5i %10.2f", tas::genps_id().at(i), tas::genps_id_mother().at(i), tas::genps_p4().at(i).pt(), tas::genps_isMostlyLikePythia6Status3().at(i), tas::genps_status().at(i), tas::genps_p4().at(tas::genps_idx_mother().at(i)).M()) << endl;
      if (abs(tas::genps_id().at(i)) == 13 || abs(tas::genps_id().at(i)) == 11){
        cout << "  --> " << tas::genps_isPromptFinalState().at(i)                        << endl;                           
        cout << "  --> " << tas::genps_isPromptDecayed().at(i) << endl;                             
        cout << "  --> " << tas::genps_isDirectPromptTauDecayProductFinalState().at(i) << endl;     
        cout << "  --> " << tas::genps_isHardProcess().at(i)  << endl;                              
        cout << "  --> " << tas::genps_fromHardProcessFinalState().at(i) << endl;                   
        cout << "  --> " << tas::genps_fromHardProcessDecayed().at(i) << endl;                      
        cout << "  --> " << tas::genps_isDirectHardProcessTauDecayProductFinalState().at(i) << endl;
        cout << "  --> " << tas::genps_fromHardProcessBeforeFSR().at(i) << endl;                    
      }
    }
  }
